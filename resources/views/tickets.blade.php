...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <a href="ticket-category/create" class="ml-4 font-normal text-lg text-blue-400 hover:underline hover:text-blue-600 cursor-pointer">
                Manage Categories 
                ...
                <div class="p-6">
                    <div class="mb-6 flex justify-between items-center flex-wrap">
                        <div class="flex space-x-4 select-none">
                            <div class="ticket-tab-container">
                                ...
                                @foreach ([
                                    'allTickets' => ['label' => 'All', 'count' => $dumps->count(), 'icon' => 'albums'],
                                    'unread' => ['label' => 'Unassigned', 'count' => $unreadTickets->count(), 'icon' => 'mail-unread'],
                                    'assigned' => ['label' => 'Assigned', 'count' => $assignedTickets->count(), 'icon' => 'person'],
                                    'closed' => ['label' => 'Closed', 'count' => $closedTickets->count(), 'icon' => 'checkmark-circle'],
                                ] as $id => $data)
                        ...
                        <!-- Add this right after the search input in the top controls -->
                        <div class="flex space-x-4 items-center">
                            <div class="relative">
                                <select id="level-filter" class="px-4 pr-8 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 ease-in-out">
                            ...
                            <input type="text" id="ticket-search" placeholder="Search ticketss..." class="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 ease-in-out">
                        </div>
                    ...
                    <div class="overflow-x-auto select-none">
                        <div class="bg-white overflow-y-auto overflow-x-hidden table-data" style="height: 70vh;">
                            @foreach ([ 'allTicketsbox' => $dumps, 'unreadbox' => $unreadTickets, 'assignedbox' => $assignedTickets, 'closedbox' => $closedTickets ] as $boxId => $ticketList)
                                <div class="ticket-content {{ $boxId === 'unreadbox' ? '' : 'hidden' }}" id="{{ $boxId }}">
                                    ...
                                    <a href="{{ route('ticket.show', ['ticket' => $ticket->id]) }}" 
                                        ...
                                        {{-- make this disappear from half web screen width to phone screen width --}}
                                        <div class="w-1/4 py-4 px-6 whitespace-nowrap md:block hidden max-md:hidden">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10">
                                                    @if(isset($ticket->user) && empty($ticket->user->profile_picture))
                                                        <img class="h-10 w-10 rounded-full object-cover" src="https://ui-avatars.com/api/?name={{ urlencode($ticket->user->name) }}&color=7F9CF5&background=EBF4FF" alt="{{ $ticket->user->name }}" style="border: 1px solid blue">
                                                    @elseif(isset($ticket->user))
                                                        <img class="h-10 w-10 rounded-full object-cover" src="{{ asset($ticket->user->profile_picture) }}" alt="{{ $ticket->user->name }}">
                                                    @else
                                                        <img class="h-10 w-10 rounded-full object-cover" src="https://ui-avatars.com/api/?name={{ urlencode($ticket->guest_name) }}&color=7F9CF5&background=EBF4FF" alt="{{ $ticket->guest_name }}" style="border: 1px solid blue">
                                                    @endif
                                                </div>
                                                <div class="ml-4">
...
<script>
    $(document).ready(function() {
        ...
        $('.ticket-tab').click(function(e) {
            ...
            filterTickets($('#ticket-search').val().toLowerCase(), selectedLevel);
        ...
        $('#level-filter').on('change', function() {
            ...
            var searchTerm = $('#ticket-search').val().toLowerCase();
            filterTickets(searchTerm, selectedLevel);
        });

        function filterTickets(searchTerm, level = 'all') {
            ...
            $('#' + activeTab + ' .clickable').each(function() {
                ...
                var ticketLevel = row.data('level');

                // Remove existing highlights
                row.find('.highlight').contents().unwrap();

                var matchesSearch = ticketId.includes(searchTerm) || 
                                  userName.includes(searchTerm) || 
                                  subject.includes(searchTerm) || 
                                  content.includes(searchTerm);
                
                var matchesLevel = level === 'all' || ticketLevel === level;
