        ...
        <div class="bg-gray-800 h-screen border-r border-gray-500 navigations hidden sm:block md:block lg:block xl:block sm:w-20 md:w-20 lg:w-64 xl:w-64">
            <div id="side-bar" class="flex flex-col mt-4">
                ...
                <div class="shrink-0 flex items-center mx-3 px-2 pb-4 border-b-2 border-gray-500 mr-3">
                    <a href="{{ route('admindashboard') }}" class="flex items-center gap-3">
                        <img src="{{ asset('images/system logo no bg 3.png') }}" alt="PSU Logo" class="h-10 w-10 border-2 border-green-500 rounded-full scale-[1.2]" />
                        <span class="text-xl font-semibold tracking-wide text-white">Student</span>
                    </a>
          </div>
                ...
                <nav class="flex-grow space-y-2 p-3">
                    <x-nav-link :href="route('dashboard')" :active="$currentRoute === 'dashboard'">
                        <i class="fas fa-home my-auto mr-5"></i>
                        <span>{{ __('Dashboard') }}</span>
                    </x-nav-link>

                    <x-nav-link :href="route('chatbot')" :active="$currentRoute === 'chatbot'">
                        <i class="fas fa-robot my-auto mr-5"></i>
                    ...
                    <x-nav-link :href="route('my-tickets')" :active="$isActiveTickets">
                        <i class="fas fa-ticket-alt my-auto mr-5"></i>
                        <span>{{ __('My Tickets') }}</span>
                        @if ($myTotalTickets == 0)
                        @else
                            <span class="absolute right-2 bg-red-500 text-sm text-white px-2 rounded-full ml-24">{{$myTotalTickets}}</span>
                        @endif
                    </x-nav-link>
                    {{-- Add logic to set active state for specific link --}}
