                ...
                <div class="px-6 py-4">
                    <form action="{{ route('users.update', $user->id) }}" method="POST">
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <x-primary-button type="submit" class="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold mr-2 py-2 px-4 rounded-md">
                                    Save Changes
                                </x-primary-button>
                                <form action="{{route('users.destroy', $user->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <x-primary-button type="submit" class="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-md">
                                        Delete Account
                                    </x-primary-button>
                                </form>
                            </div>
                            ...
                            <a href="http://127.0.0.1:8000/UserManagement" class="text-gray-500 hover:text-indigo-600 font-semibold">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
